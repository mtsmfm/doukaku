...
def solve(input)
  down_cards_list = input.split(?,).map(&:chars)

  number_range = 1..down_cards_list.first.size
  width = down_cards_list.first.size
  height = down_cards_list.size
  numbers = (number_range.to_a * height).sort

  candidates = down_cards_list.map do |down_cards|
    numbers.combination(width).map(&:sort).uniq.select do |cards|
      match?(down_cards, cards)
    end
  end

  candidates[0].product(*candidates[1..-1]).select {|list|
    list.flatten.sort == numbers
  }.map {|list| list.join(?,) }.join(?|)
end

def match?(down_cards, cards)
  (down_cards == cards.map {|c| c.odd? ? ?o : ?x })
end
