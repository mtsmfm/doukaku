...
class N
  ...
  def children
    ((@n + 1)...@b).map {|nn| N.new(b: @b, n: nn, parent: self) }
  end

  def to_s
    @parent.to_s + @n.to_s(@b)
  end
end

def solve(input)
  b, m = input.split(?,).map(&:to_i)
  numbers = (1...b).map {|n| N.new(b: b, n: n) }

  children = numbers.flat_map(&:children)

  until children.empty? || numbers[m - 1]
    numbers += children
